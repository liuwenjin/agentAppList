  ...
  <div class="pptAppContent" :style="style">
    <div class="contentInfoArea">
      <span v-if="content.title" class="titleItemArea">
        <h1 class="ellipsis" :title="content.title" from="title" v-html="content.title"></h1>
      </span>
      <div :style="{
          height: content.title ? 'calc( 100% - 150px )' : '100%',
        ...
        <div class="confirmOptionsArea exportHiddenItem" v-if="index !== 2">
          <el-button v-if="index === 1" @click="handleClickBack()" class="backBtn" text type="info" size="large"><i class="bi bi-chevron-double-left"></i>{{ $t("back") }}</el-button>
          <el-button @click="handleLightTree(1)" class="lightTreeBtn exportHiddenItem" size="large" type="success">{{ buttonText[index] }}</el-button>
        </div>
      </div>
    </div>
  </div>
      ...
      updateLightData(qMap, json) {
        ...
        if (json.name && qMap && qMap[k]) {
          ...
          for (let i = 0; i < tArr.length; i++) {
            json.value = json.value && tArr[i].selected === (tArr[i].choices[tArr[i].answer].name || tArr[i].choices[tArr[i].answer]);
          }
        } else {
          delete json.value;
        }
