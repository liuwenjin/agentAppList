      ...
      <div class="lightedInfo">
        ...
        <div class="resultInfoArea">
          <span
            ...
            v-if="result.max !== result.value"
            v-html="resultTips"
          ></span>
          <span v-else>{{ $t("successTips") }}</span>
          <span class="scoreInfoItem"
            >{{ scoreTips + ": " + result.score }} (
            {{ totalTips + ": " + result._score }} )</span
          >
          <el-button
...
`,
      ...
      - 'anwsers': 记录问题信息和学员回答信息的数组, 每个元素的字段包括：
          - 'name': 字符串，题目的描述。
          - 'type': 枚举类型，题目的类型可以为'单选题', '多选题', '填空题'。
          - 'knowledgePoint': 题目考察的知识点。
          - 'choices': 一个数组，包含备选项。
          - 'answer': 数字，题目的正确选项的编号。
          - 'selected': 数组，答卷中考生的选项内容。
          - 'inputContent': 字符串, '填空题'的考生回答内容。
      ...
      - 'explanations', 一个数组，包含对错题（上下文中 anwsers里score为0的项）的讲解内容，讲解内容包含如下几个字段:
          ...
          - 'content': 字符串，错题分析内容，说明可能的错误原因和易混淆之处。
          - 'solving': 字符串，解题思路，详细讲解正确答案的解题步骤和逻辑，帮助学生理解解题方法;
          - 'review': 在解析中复习相关知识点，强化学生对概念的理解和记忆。
          - 'mistakes': 指出学生可能犯的常见错误，并提供避免这些错误的建议。
...
Example of results: 
\`\`\` 
{ "reviews": [ "xxx...", "xxx..." ], "explanations": [{ "name": "xxx...", "answer": "xxx...", "selected": "xxx...", "content": "xxx..." }], "suggestions": [{ "title": "xxx...", "description": "xxx..." }, { "title": "xxx...", "description": "xxx..." }], "summary": [ "xxx...", "xxx..." ]} 
\`\`\`
      `,
      qMap: {},
      analysisText: "",
    ...
    plugin: {
      mounted: function (el, param) {
        ...
        webCpu.updateView(el, app, function (c, d, t) {
          t.chart.on("click", (params) => {
            let arr = params.data.questions;
            let item = arr[0];
            let title = item.point.replace("_", "-");
            let text = _self.generateQuestionContent(title, arr);
            _self.displayQuestionList(text, title);
            console.log(params);
          });
        });
  ...
  methods: {
    ...
    updateCountInfo() {
      this.result = this.countAnswerInfo();
    ...
    displayQuestionList(text, title) {
      let app = {
      ...
      let callback = function () {
        ...
        // });
        let tElem = $(app.card.task.container).find(".articleContentItem")[0];
        html2canvas(tElem).then((tCanvas) => {
          const iData = tCanvas.toDataURL("image/png");
          // 创建一个临时的a标签
      ...
      webCpu.CardItem.renderCardDialog(
        ...
        app,
        {
          ...
          title: webCpu.messages[lang].questionListTips,
          titleHeight: 50,
          titleStyle: {
          ...
          // menu: [
          //   {
          //     label: `<span style='font-size: 20px; padding: 0px 10px;'>${webCpu.messages[lang].copyBtn}</span>`,
          //     action: callback,
          //   },
          // ],
        },
      ...
      webCpu.CardItem.renderCardDialog(
        ...
        app,
        {
          ...
          menu: [
            ...
            {
              label: `<span style='font-size: 20px; padding: 0px 10px;'>${webCpu.messages[lang].regenerateContent}</span>`,
              action: callback1,
            },
          ],
        },
    ...
    generateQuestionContent(title, arr) {
      let str = `### ${title} \n\r`;
      let tHead = ["A", "B", "C", "D", "E", "F"];
      for (let i = 0; i < arr.length; i++) {
        let item = arr[i];
        ...
        if (item.type === "单选题" || item.type === "多选题") {
          ...
          let correct = item.answer.map((item) => {
            return tHead[item];
          });
          let yours = tItem.selected || [];
          yours = yours.map((item) => {
            let index = cArr.indexOf(item);
            return tHead[index];
          });
          t += `**正确选项**：${correct.join(", ")}\n\r`;
          t += `**你的选项**：${yours.join(", ")} 【得分: ${
            tItem.score
          }(满分: ${tItem._score})】\n\r`;
          t += `**题目解读**：${tItem.reason || "暂无"}\n\r`;
          t += `**解题思路**: ${tItem.problemSolving}`;
        } else if (item.type === "填空题") {
          t += `**参考答案**：${tItem.answer.join("; ")}\n\r`;
          t += `**你的回答**：${tItem.inputContent} 【 得分: ${tItem.score}(满分: ${tItem._score})】\n\r`;
          t += `**评分理由**：${tItem.evaluate}\n\r`;
          t += `**解题思路**: ${tItem.problemSolving}`;
        } else {
          let tArr = tItem.answer.map((item) => {
            return item.text;
          });
          t += `**参考答案**：${tArr.join("; ")}\n\r`;
          t += `**你的回答**：${tItem.inputContent} 【 得分: ${tItem.score}(满分: ${tItem._score})】\n\r`;
    ...
    transformJson(json) {
      ...
      //     t = `- **${lang === "zh" ? "错题分析：" : "Answer Analysis"}**\n\r ${
      //       item.content
      //     }\n\r`;
      ...
      //     t = `- **${lang === "zh" ? "解题思路：" : "Problem Solving"}**\n\r ${
      //       item.solving
      //     }\n\r`;
      ...
      //     t = `- **${
      //       lang === "zh" ? "知识点回顾：" : "Knowledge point review"
      //     }**\n\r ${item.review}\n\r`;

      //     str += t;
      //     this.analysisText += t;

      //     t = `- **${lang === "zh" ? "常见错误：" : "Common Mistakes:"}**\n\r ${
      //       item.mistakes
      //     }\n\r`;
