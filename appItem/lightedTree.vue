    ...
    <div class="contentInfoArea">
      <div class="lightedInfo">
        <div class="pageHeader">
          <span class="treeNameSpan" style="float: left">{{ $t("knowledgeTree") }}:
            <span class="ellipsis" style="max-width: 400px; margin-top: -4px" :title="root.name">{{ root.name }}</span></span>
          <span class="submitterSpan" style="float: right">{{ $t("submitter") }}: {{ documentMeta.userName || "匿名" }}
            <span style="margin-left: 20px">{{ submitTime }}: <span>{{ time }}</span></span></span>
        </div>
        <div class="resultInfoArea">
          <span class="resultTipsArea" v-if="needed !== count.value" v-html="resultTips"></span>
          <span v-else>{{ $t("successTips") }}</span>
          <span class="scoreInfoItem">{{ scoreTips + ": " + count.score }} ( {{ totalTips + ": " + count.sum }} )</span>
          <el-button @click="viewSuggestions()" type="primary" size="large" :disabled="loadingStatus" :loading="loadingStatus" class="viewSuggestionsBtn exportHiddenItem">
            <slot v-if="context[2] && context[2].analysisText">{{
              viewResult
            }}</slot>
            <slot v-else>
              <el-icon v-if="!loadingStatus" style="margin-right: 5px" :size="25">
                <magic-stick />
              </el-icon>{{ viewerBtn }}
            </slot>
          </el-button>
          <span v-if="!(context[2] && context[2].analysisText)" class="pointsCostTips exportHiddenItem">{{ $t("pointsCostTips") }}</span>
        </div>
      </div>
      <div class="myContentArea">
        <div class="dslInfoArea" v-if="content.dsl && content.dsl.length !== 0">
          <div class="dslInfoItem" :key="index" v-plugin="{ app: d }" v-for="(d, index) in content.dsl"></div>
        </div>
      </div>
    </div>
  </div>
...
Example of results: \`\`\` { "reviews": [ "xxx...", "xxx..." ], "explanations": [{ "name": "xxx...", "answer": "xxx...", "selected": "xxx...", "content": "xxx..." }], "suggestions": [{ "title": "xxx...", "description": "xxx..." }, { "title": "xxx...", "description": "xxx..." }], "summary": [ "xxx...", "xxx..." ], } \`\`\`
    ...
    beforeMount() {
      if (this.content && this.content.dsl && this.content.dsl[0] && this.content.dsl[0].data) {
    ...
    computed: {
      root() {
        let ret = {};
        if (this.content.dsl && this.content.dsl[0] && this.content.dsl[0].dsl && this.content.dsl[0].dsl.data) {
          ret = this.content.dsl[0].dsl.data[0];
        }
        return ret;
      },
      time() {
