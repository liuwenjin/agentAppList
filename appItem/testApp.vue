    ...
    <div class="contentInfoArea">
      <div class="stepInfoArea">
        ...
        <span class="switchButtonList">
          <el-button :disabled="index === 0" @click="switchQuestionItem(-1)" type="info" plain round><i class="bi bi-chevron-left"></i>{{ $t("lastQuestion") }}</el-button>
          <el-button :disabled="index === list.length - 1" @click="switchQuestionItem(1)" type="primary" plain round>{{ $t("nextQuestion") }}<i class="bi bi-chevron-right"></i></el-button>
        </span>
      </div>
      ...
      <div v-if="list[index]" class="questionInfoArea">
        <div :selected="
          ...
          " @click="handleSelectItem(i, index)" class="choiceItem" v-for="(d, i) in list[index].choices" :key="i">
